...
public class ArtifactorySearchService extends AbstractRESTfulVersionReader implements IVersionReader {
     ...
     * 替换构建正式的路径[qq]
     */
	private static final String REPLACE_CONTENT = "api/storage/";
	...
	@Override
	public Set<String> callService(String pGroupId,
                                   String pArtifactId,
                                   String pPackaging,
                                   String pVersion,
                                   String pRepos,
		...
		final MultivaluedMap<String, String> requestParams
                = RESTfulParameterBuilder.create(pGroupId, pArtifactId, pPackaging, pVersion, pRepos, pClassifier);
		...
		try {
			...
			for (ArtifactoryResultEntryModel current : fromJson.getResults()) {
				...
				if (validPackaging(current.getUri(), pPackaging)) {
					retVal.add(current.getUri().replace(REPLACE_CONTENT, StringUtils.EMPTY));
				}
			}
		} catch (Exception e) {
