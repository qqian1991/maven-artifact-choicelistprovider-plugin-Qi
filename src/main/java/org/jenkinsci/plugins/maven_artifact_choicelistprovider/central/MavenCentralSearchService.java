...
public class MavenCentralSearchService implements IVersionReader {
	 ...
	 * @param pClassifier c
	 * @return result
	 * @throws VersionReaderException exception
	 */
	@Override
	public List<String> retrieveVersions(String pGroupId,
										 String pArtifactId,
										 String pPackaging,
										 String pVersion,
										 String pRepos,
		...
		try {
			final String targetURL = createURL(pGroupId, pArtifactId, pPackaging, pVersion, pRepos, pClassifier);
	...
	/**
	 * 
	 * @param pReposnseEntry
	 * @param pRequestedPackaging
	 * @return The items to add, but never NULL.
	 */
	List<String> createItemURLs(final ResponseDoc pReposnseEntry, final String pRequestedPackaging) {
		final List<String> retVal = new ArrayList<String>();
		final StringBuilder sb = new StringBuilder();

		// base url
		sb.append(getRetrieveURL());
		sb.append(pReposnseEntry.getGroupId().replace(".", "/")).append("/");
		sb.append(pReposnseEntry.getArtifactId()).append("/");
		sb.append(pReposnseEntry.getVersion()).append("/");

		// artifact
		sb.append(pReposnseEntry.getArtifactId()).append("-");
		sb.append(pReposnseEntry.getVersion());

		// packaging
		for (String currentEC : pReposnseEntry.getEc()) {
	...
	boolean containsResponses(final MavenCentralResponseModel pReposnse) {
		if (pReposnse == null) {
			return false;
		}

		if (pReposnse.getResponse() == null) {
			return false;
		}

		if (pReposnse.getResponse().getDocs() == null) {
			return false;
		}

		return pReposnse.getResponse().getDocs().length > 0;
	}
     ...
     * @return result
     */
	private String createURL(final String pGroupId,
        ...
        if (!"".equals(pVersion)) {
            targetURL.append("a:\"" + pVersion + "\"+AND+");
        }

        if (!"".equals(pRepos)) {
            targetURL.append("a:\"" + pRepos + "\"+AND+");
        }
